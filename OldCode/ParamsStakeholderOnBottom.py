...
filename = 'ParamsStakeholderOnBottom'
...
target_type = 'circle' #note that the default start position for circle is right side, while default for figure8 is center.
v0 = 1E-6                               #loiter velocity, m/s
xt = length/2                           #center of the circle
yt = length/2
rd = 3.0E-5                             #desired loiter distance, m
initial_position = np.array([[xt,yt-2*rd],[xt+rd,yt]]) #initial positions of particles
...
Brown = 0                            #Brownian motion. Set to 1 for on or 0 for off
...
n_vision = 30         #prediction horizon
...
ub_e_factor = 20       #multiple of uemax to set upper bound at. 1 is normal for a single particle.
