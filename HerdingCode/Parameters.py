...
v0_herder = 5E-6        # The speed the herder particle moves at. #was previously 20
particle_radius = 4E-6  # m, radius of colloidal particle
herder_radius = 4e-6    # m, radius of the herder particle

initial_position = np.zeros((11,2))  #The initial positions of particles (both passive and herders).
...
#The first n_herders entries are the initial positions of the herders.
n_particles = len(initial_position)    #The number of particles (both passive and herders).
...
def matchTargets(target_position_unsorted):   
    distances = np.zeros((n_particles - n_herders, n_particles - n_herders))  # Number of particles designated as herder particles. Herders are the first elements of initial_position.
...
reaction_rate = 0.010 # mol/s/m^2, 0.01 is typical of platinum in H2O2. So this is 1/3 of the max physically reasonable.
...
therder = ell_target / v0_herder
tchased =  d_tol * R_fh / kdiff
#ttotal = (n_particles-n_herders)*(tchased+therder) # this n_particles is not the same number used in the paper because this contains herder particles too.
...
#dbrown2 = 4*Dp*(n_particles-1)*R_fh/kdiff #this n_particles is not the same number used in the paper because this contains herder particles too.
if d_tol < dbrown2:
